---
name: Google API専用 画像生成スキル
description: Google APIのみを使用してアニメ系イラストを生成する。Nano Banana 2を優先し、Google API以外へのフォールバックは行わない。
---

# Google API専用 画像生成スキル
...
- `GOOGLE_API_KEY` が以下のいずれかの `.env` に設定されていること:
  - `c:\Obsidian Vault\demo\05_設定\.env`
  - `c:\Obsidian Vault\X_Post_System\2_Config\.env`
...
- `c:\Obsidian Vault\demo\03_Remotion\public\images\` — Remotion用
- 画像形式: PNG
...
| `regen_split_image.py` | 個別画像再生成（対比カット等） |
| `generate_manga.bat` | Windows用バッチファイル |
